...
#         vit_output = self.vit(image, mask=None)  # Assuming no mask is needed
#         return vit_output
...
class StylePyramidNet(nn.Module):
    def __init__(
        ...
        self.return_only_vit = return_only_vit
    ...
    def forward(self, hint, sample=None, timesteps=None, encoder_hidden_states=None):
        ...
        for i, stage in enumerate(self.stages):
            ...
            h_out = stage(h_in)
            # print("shape of h_out ----------------------->", h_out.shape)
        ...
        combined_features = torch.cat([o.flatten(1) for o in outs], dim=1)
        # print("Combined features shape: **************************", combined_features.shape)
        # print("vision encoder shape -=-=-=-=-=-=-=-=-", outs[-1].shape)
